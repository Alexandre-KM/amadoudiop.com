													...
													{if $option.unique_carrier}
														{foreach $option.carrier_list as $carrier}
															<strong>{$carrier.instance->name|escape:'htmlall':'UTF-8'}</strong>
															  {if $carrier.instance->name == 'Contrat hebergement'}
																	{include file="./contrat-hebergement-signature.tpl"}
																{/if}
																{if $carrier.instance->name == 'Contrat Prestastions Techniques'}
																 {include file="./contrat-intervention-technique-signature.tpl"}
															 {/if}
														{/foreach}
													{/if}
