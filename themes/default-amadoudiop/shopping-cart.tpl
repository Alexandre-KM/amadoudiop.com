	...
	{if !$advanced_payment_api && ((!empty($delivery_option) && (!isset($isVirtualCart) || !$isVirtualCart)) OR $delivery->id || $invoice->id) && !$opc}
		<div class="order_delivery clearfix row">
			{if !isset($formattedAddresses) || (count($formattedAddresses.invoice) == 0 && count($formattedAddresses.delivery) == 0) || (count($formattedAddresses.invoice.formated) == 0 && count($formattedAddresses.delivery.formated) == 0)}
				...
				{/if}
			{else}
				{foreach from=$address_list	 key=k item=address}
					<div class="address col-xs-12 col-sm-6">
								...
								{$address.company|escape:'html':'UTF-8'}<br />
								{$address.address1|escape:'html':'UTF-8'}<br />
								{if isset($address.address2)}
									{$address.address2|escape:'html':'UTF-8'}<br />
								{/if}
								{$address.postcode|escape:'html':'UTF-8'}<br />
								{$address.city|escape:'html':'UTF-8'}<br />
								{$address.country|escape:'html':'UTF-8'}<br />
					</div>
				{/foreach}
			{/if}
		</div>
